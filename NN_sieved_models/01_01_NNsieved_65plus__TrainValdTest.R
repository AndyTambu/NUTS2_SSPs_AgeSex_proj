...
brulee_grid <- my.grid <- expand.grid( learn_rate = 10^(seq(-5, 0, length.out = 8)), 
                                       hidden_units = seq(5,48, by = 3),
...
Sieve_fit_brulee_first_test_corct_funcs_65plus_test20022025_corrStu <- lapply(rand_samp,function(z){
...
#save(Sieve_fit_brulee_first_test_corct_funcs_65plus_test20022025_corrStu, file = './results/Sieve_fit_brulee_first_test_corct_funcs_65plus_test20022025_corrStu.RData')
load(file = './results/Sieve_fit_brulee_first_test_corct_funcs_65plus_test20022025_corrStu.RData')
lapply(Sieve_fit_brulee_first_test_corct_funcs_65plus_test20022025_corrStu, function(x) sapply(x, length))
...
result_tbbl <- as_tibble(result_df)%>% 
  ...
  ungroup()
# this is the version where we condier the median of the row as the estimate which is actually the thing which
...
assign(paste0('result_tbbl_', '65plus'), 
       as_tibble(result_df) %>% 
         rowwise() %>% 
...
save(list = paste0('result_tbbl_', '65plus'), 
     file = paste0('./01_Age_Structure/AAA_longer_timeSeries/results/', 
                   paste0('result_tbbl_', '65plus'), '.RData'))



ggplot(result_tbbl_65plus, aes(x = id)) +
...
result_tbbl_rescaled <- 
  ...
  cbind(true_val = test_data$lag1_quot_65) %>% #the variable need to be changed according to the age group.
  cbind( ctry_prop = 
           test_data %>% 
           # dplyr::filter(age.group == 'under15') %>% 
           dplyr::select(ctry_prop_65) %>% pull()
  ) %>% 
