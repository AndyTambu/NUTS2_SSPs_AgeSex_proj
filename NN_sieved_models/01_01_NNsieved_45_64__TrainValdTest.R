...
#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
set.seed(86)                                    # <- Ensure reproducibility
#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
...
# to_drop_clean <- c(
# 
#  "lag2_quot_15_24"   ,      "lag1_g_15_24"           ,
#  "lag1_ctry_prop_45_64",    "lag1_25_44"             ,
# "lag1_ctry_prop_25_44" ,   "lag1_ctry_prop_15_24"   ,
# "lag2_15_24"            ,  "lag1_15_24"             ,
# "lag1_ctry_prop_45_64"   , "lag1_ctry_prop_25_44"   ,
# "lag1_g_65"            ,   "lag1ratio.tot_pop_NUTS2",
# "lag1_g_25_44"         ,   "lag2_ctry_prop_25_44"   ,
# "lag1_ctry_prop_25_44" )
...
to_drop_clean <- c(to_drop_clean,c(
  'lag1_quot_15_24',
  'lag1_ctry_pop_g_15_24',
  'ctry_prop_45_64' ,  
  'lag1_25_44'   ,
  'lag2_15_24'  ,
  'lag1_ctry_pop_g_45_64',
  'lag1_15_24'   ,
  'lag1_ctry_prop_45_64',
  'lag1_ctry_prop_15_24',
  'lag1_ctry_prop_15_24',
  'lag1_ctry_pop_g_under15',
  'lag1_ctry_prop_25_44',
  'lag1_ctry_pop_g_65',
  'lag2_ctry_prop_25_44',
  'lag1_ctry_prop_25_44',
  'lag1_quot_25_44',
  'ctry_prop_under15',
  'lag1_ctry_prop_15_24',
...
brulee_grid_II <- my.grid <- expand.grid( 
  learn_rate = 10^seq(-5, -1, length.out = 10), 
...
# 20022025
# A tibble: 270 × 5
# Groups:   hidden_units, activation, learn_rate [270]
# hidden_units activation learn_rate   rmse   rsq
# <dbl> <chr>           <dbl>  <dbl> <dbl>
#       10     elu            0.0359 0.0108 0.928
...
set.seed(1)
rand_samp <- sample(seq(30,79), 20)


start = Sys.time()


Sieve_fit_brulee_first_test_corct_funcs_45_64_test20022025_corrStu <- lapply(rand_samp,function(z){
...
#save(Sieve_fit_brulee_first_test_corct_funcs_45_64_test20022025_corrStu, file = './results/Sieve_fit_brulee_first_test_corct_funcs_45_64_test20022025_corrStu.RData')
load(file = './results/Sieve_fit_brulee_first_test_corct_funcs_45_64_test20022025_corrStu.RData')
lapply(Sieve_fit_brulee_first_test_corct_funcs_45_64_test20022025_corrStu, function(x) sapply(x, length))
...
# this is the version where we condier the MEDIAN of the row
# as the estimate which is actually the thing which
# we are goign to use then in the proper modelling effort.
# it is usually nice if takign this values improves the 
# situation of our estimates instead of making less good :)
...
ggplot(result_tbbl_45_64, aes(x = id)) +
...
result_tbbl_rescaled <- 
  ...
  cbind(true_val = test_data$lag1_quot_45_64) %>% #the variable need to be changed according to the age group.
  cbind( ctry_prop = 
           test_data %>% 
           # dplyr::filter(age.group == 'under15') %>% 
           dplyr::select(ctry_prop_45_64) %>% pull()
  ) %>% 
...
# the rescaled-R2 is also usually higher than the one of the direct estimates since it is 
# coming from a set which has more variance and also is less sensitive to the micro 
# variations which we are studying here.
...
test_r2 = 1-(mse_test/var(test_data$y))
test_r2
# ok we perform better than repeater :) 
...
# new definiton of the mean. TIME DEPENDENT MEAN

test_data_raw %>% 
  dplyr::mutate(period = factor(period, levels = c("2000-2004","2005-2009","2010-2014","2015-2019","2020-2024")))

mse_test =  mean((
rbind(full_testData_raw,full_trainingData_raw) %>% 
  dplyr::mutate(period = factor(period, levels = c("2000-2004","2005-2009","2010-2014","2015-2019","2020-2024"))) %>% 
  dplyr::select(geo, period, y, lag1_quot_45_64) %>% 
  dplyr::rename(y_old = y) %>% 
  # pivot_wider(names_from = period, values_from = lag1_quot_45_64) %>% 
  right_join(
    test_data_raw %>% 
               dplyr::mutate(period = factor(period, levels = c("2000-2004","2005-2009","2010-2014","2015-2019","2020-2024"))) %>% 
               dplyr::select(geo,y,period) %>% 
               dplyr::rename(period_test = period), by = "geo"
  ) %>% 
  mutate(period = as.numeric((period)), 
         period_test = as.numeric((period_test))) %>%
  filter(period <= period_test) %>%  # Keep only rows where period < period_test
  dplyr::group_by(geo) %>% 
  group_by(geo, period_test,y) %>%
  summarise(lag1_quot_45_64_PrevMean = mean(lag1_quot_45_64) ) %>% 
  ungroup() %>% 
  mutate(diff2 = (y-lag1_quot_45_64_PrevMean)^2) %>% 
  pull(diff2)))
